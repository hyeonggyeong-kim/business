...
function renderFamilyApp() {
    ...
    screen.innerHTML = `
        ...
        <div class="chat-list" id="familyChatList">
            ${messagesHtml}
        </div>
        ...
        <div class="input-area">
            <input type="text" class="msg-input" id="familyMsgInput" placeholder="메시지 입력" 
                   onkeypress="if(event.key==='Enter') sendFamilyMsg()">
            <button class="send-btn" onclick="sendFamilyMsg()">전송</button>
        </div>
    `;
    
    setTimeout(() => {
        const chatList = document.getElementById('familyChatList');
